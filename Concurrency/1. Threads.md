...
If `launchThread` ended and C++ just quietly destroyed the `t` object while the OS thread was still running `myTask()`, that OS thread would become an "orphan." If `myTask()` tried to access variables that used to exist in `launchThread` (e.g., passed by reference, (read Functions to understand how)), it would read deleted memory, causing undefined behaviour and silent data corruption.
