...
Inline variables (which can have external linkage without causing ODR violations) were introduced in C++17.
...
Note that you can define external variables only at namespace scope (including global scope) and not inside block scope. But forward declaration can be done anywhere including block scope.
...
Note that function forward declarations don’t need the `extern` keyword , the compiler is able to tell whether you’re defining a new function or making a forward declaration based on whether you supply a function body or not. Variables forward declarations  need the `extern` keyword to help differentiate uninitialised variables definitions from variable forward declarations (they look otherwise identical):
...
Warning : If you want to define an uninitialised non-const global variable, do not use the extern keyword, otherwise C++ will think you’re trying to make a forward declaration for the variable.
