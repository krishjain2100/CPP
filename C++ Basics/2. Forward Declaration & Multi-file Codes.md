...
**Best practice**
When addressing compilation errors or warnings in your programs, resolve the first issue listed and then compile again, because the later errors maybe caused by the undefined behaviours branching from the first one.
...
3. Types, templates, inline functions, and inline variables are allowed to have duplicate definitions in different files, as long as each definition is identical. 

Violating ODR part 1 will cause the compiler to issue a redefinition error. 
Violating ODR part 2 will cause the linker to issue a redefinition error. 
Violating ODR part 3 will cause undefined behavioir.

Functions that share an identifier but have different sets of parameters are also considered to be distinct functions (overloading), so such definitions do not violate the ODR.
