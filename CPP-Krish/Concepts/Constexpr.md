...
- The initialiser of a constexpr variable (discussed below)
...
- Possibly: A non-constant expression where the compiler is able to determine all values at compile-time (optimised under the as-if rule).
...
A **constexpr** variable is always a compile-time constant. As a result, a constexpr variable must be initialised with a constant expression, otherwise a compilation error will result. Example
