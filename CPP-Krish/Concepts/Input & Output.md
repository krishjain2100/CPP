...
- **get()**, which simply gets a character from the input stream. Example:
...
Input: Hello!
Output: Hello! and then terminate.
...
Using `setf()` and `unsetf()` tends to be awkward, so C++ provides a second way to change the formatting options: manipulators. They hey are smart enough to turn on and off the appropriate flags. Example:
