...
int main() {
    int x{ 1 };
    int y{ 2 };
    x = y; // y is not an rvalue, but this is legal (explained below)
}
```
...
We can write a program and have the compiler tell us the kind of value something is.
...
int main() {
    ...
    PRINTVCAT("Hello");  // lvalue (this is actually const char*)
    PRINTVCAT(++x);      // lvalue
    PRINTVCAT(x++);      // rvalue
}
```
...
- Postfix Increment (`x++`) - rvalue
	1. It creates a nameless, **temporary copy** (a snapshot) of `x`'s current value.
	2. It increments the real variable `x` in the background.
	3. It returns that **nameless temporary copy**.

This implies that this works (and it does)
```cpp
int x = 0; 
++x = 2;
```
