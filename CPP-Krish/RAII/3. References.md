...
Note: We cannot use `ref1` in constant expressions because it binds to non-const value and can be changed in between some statement at runtime so it wouldn't be apparent to programmer as to what happened if the compiler allowed to use it. So only constexpr references to const globals and static locals is allowed to be used in constant expressions at compile time. 

Moreover the compiler simply abstracts these away for compile time evaluation if they are not required to be emitted (i.e not used in other code files).

Constexpr references typically don’t see much use.
...
Added in C++11,  an R-value reference is a reference that is designed to be initialized with an r-value (only). It is created using a double &, like lvalue is created using single &:
...
`Fraction(3, 5)` would normally go out of scope at the end of the expression in which it is defined but since we’re initializing an r-value reference with it, its duration is extended until the end of the block. 
...
R-value references are often used as function parameters which is useful for function overloads when you want to have different behavior for l-value and r-value arguments.
...
Literal `5` is an rvalue of type `int`, and `int x` is an lvalue of type `int`,  similarly, `int&& ref` is an lvalue of type `int&&`. So not only does `fun(ref)` call `fun(const int&)`, it does not even match `fun(int&&)`, as rvalue references can’t bind to lvalues.
