...
The program gives compilation error because the compiler compiles the contents of code files sequentially. That produces the error, _identifier not found_.
...
To write a forward declaration for a function, we use a **function declaration** statement (also called a **function prototype**). The names of the parameters can be optionally included (they are not required as they are not considered to be part of the function declaration). 
