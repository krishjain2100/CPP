...
Because variables have scope, duration, and linkage, let’s summarise in a chart:
...
When used as part of an identifier declaration, the `static` and `extern` keywords are called **storage class specifiers**. In this context, they set the storage duration and linkage of the identifier. C++ supports these active storage class specifiers:
