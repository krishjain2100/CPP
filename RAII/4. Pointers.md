...
The first is the literal `0`. Ir is the only time you can assign an integral literal to a pointer.
...
The literal `0` can be interpreted as either an integer literal, or as a null pointer literal. Itt can be ambiguous which one we intend, and in some of those cases, the compiler may assume we mean one when we mean the other. The definition of preprocessor macro `NULL` is not defined by the language standard. It can be defined as `0`, `0L`, `((void*)0)`, or something else entirely.
