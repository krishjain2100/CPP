...
The return value from `main()` is sometimes called a **status code**(exit code and return code also). The status code is used to signal whether your program was successful or not.
...
If you want to maximise portability, you should only use `0` or `EXIT_SUCCESS` to indicate a successful termination, or `EXIT_FAILURE` to indicate an unsuccessful termination.
...
Parameters not used in body function
...
```cpp
void doSomething(int /*not neccessary but recommended*/) {
