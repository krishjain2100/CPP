...
int main()
{
    ...
    [[maybe_unused]] constexpr const int& ref2 { s_x }; 
    // needs both constexpr and const
...
The value of the referred variable can still be used only if the variable is constexpr.
This only stores the memory offset of the variables in the reference.
If you use a normal reference (`auto& port = ...`), the compiler _might_ actually allocate memory for a pointer under the hood at runtime to track that alias.
The thing done by constexpr is wherever the reference is used that reference is directly replaced by the address of the variable it is referring to, saving time from actually defining the reference at runtime.
