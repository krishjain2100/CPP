...
In member functions where object of same type as the original are passed, we might prefer nonmember functions and pass the original object also as argument, to make syntax more consistent(everywhere `.` operator will be used).
...
**Side Note:**
**Remember to refer a class in another class the class being referred to must be defined or declared before that, and any function using the members of the class can obviously not work only with forward declaration  of the class as it also needs the info about members existence, so better define it after**
